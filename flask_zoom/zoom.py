...
def get_meetings(conn, cur, start=None, end=None, num_sentences=1):
    ...
    for file in os.listdir():
        ...
        if file.endswith(".txt"):
            ...
            day+=1
            # video link not working, maybe try https://developers.google.com/youtube/player_parameters#Embedding_a_Player
            video_link = "https://www.googleapis.com/youtube/v3/videos?id={}&key={}".format("UWS0kbl4rVk", youtubeconfig.APIKEY)
            ...
            # add to database
            cur.execute("INSERT INTO recordings(topic, start_time, video, transcript, text, tokens, tags, summary, visible, zoom_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, TRUE, %s) ON CONFLICT (zoom_id) DO NOTHING", ("Test Class", date, video_link, "", text, tokens, json.dumps(keywords_dict), summary, id))
            conn.commit()
